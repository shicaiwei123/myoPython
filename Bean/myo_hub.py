...
class MyoHub:
    ...
    def __init__(self, myo_num=2, tty_one=None, tty_two=None):
        ...
        self.emg_left_pool = queue.Queue()
        self.emg_right_pool = queue.Queue()
        self.imu_left_pool = queue.Queue()
        self.imu_right_pool = queue.Queue()
    ...
    def get_data(self):
        if self.myo_num == 1:
            try:
                return self.emg_left_pool.get_nowait(), self.imu_left_pool.get_nowait()
            except queue.Empty as e:
                return None, None
        elif self.myo_num == 2:
            try:
                return self.emg_left_pool.get_nowait(), self.imu_left_pool.get_nowait(), self.emg_right_pool.get_nowait(), self.imu_right_pool.get_nowait()
            except queue.Empty as e:
