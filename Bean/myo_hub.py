...
class MyoHub:
    ...
    def __init__(self, myo_count=2):
        ...
        self.myo_list = []
        while len(self.myo_list) != self.myo_count:
            self.myo_list = self.scan_myos(myo_count=myo_count, scan_time=5.0, iface=0)
        ...
        # when self.myo_count == 1, use following queue to store emg data and imu data
                ...
                try:
                    ...
                    if self.myo_count == 1:
                        if data_packet.data_type == MyoDataType.EMG_DATA:
                            self.emg_queue.put(data_packet.data)
                        elif data_packet.data_type == MyoDataType.IMU_DATA:
                            self.imu_queue.put(data_packet.data)
